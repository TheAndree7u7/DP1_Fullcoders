...
public class PedidoFactory {
        ...
        /**
         * Crea un pedido a partir de una línea de texto y el nombre del archivo.
         *
         * @param line          Línea de texto con los datos del pedido
         * @param nombreArchivo Nombre del archivo para extraer el mes correcto
         * @return instancia de Pedido creada
         */
        public static Pedido crearPedido(String line, String nombreArchivo) throws InvalidDataFormatException {
                String[] partes = line.split(":");
                if (partes.length != 2) {
                        throw new InvalidDataFormatException(
                                        "Formato de línea de pedido incorrecto. Se esperaban 2 partes separadas por ':'. Línea: "
                                                        + line);
                }

                // Extraer el mes del nombre del archivo (data/pedidos/ventas202501.txt -> 01)
                String nombreArchivoSimple = nombreArchivo.substring(nombreArchivo.lastIndexOf("/") + 1);
                String mesArchivo = nombreArchivoSimple.substring(10, 12);
                LocalDateTime fechaRegistro = Herramientas.readFechaConMes(partes[0], mesArchivo);
                ...
                try {
                        ...
                        if (!m3Str.toLowerCase().endsWith("m3")) {
                                throw new InvalidDataFormatException(
                                                "Formato de volumen (m3) incorrecto. Debe terminar con 'm3'. Valor: "
                                                                + m3Str + ". Línea: "
                                                                + line);
                        }
                        ...
                        if (!horaLimiteStr.toLowerCase().endsWith("h")) {
                                throw new InvalidDataFormatException(
                                                "Formato de hora límite (h) incorrecto. Debe terminar con 'h'. Valor: "
                                                                + horaLimiteStr
                                                                + ". Línea: " + line);
                        }
                        horaLimite = Integer
                                        .parseInt(horaLimiteStr.substring(0, horaLimiteStr.toLowerCase().indexOf('h')));
                        ...
                        if (horaLimite <= 0)
                                throw new InvalidDataFormatException(
                                                "La hora límite debe ser positiva. Valor: " + horaLimite + ". Línea: "
                                                                + line);

                } catch (NumberFormatException e) {
                        throw new InvalidDataFormatException(
                                        "Error al parsear valores numéricos en datos del pedido: " + partes[1]
                                                        + ". Línea: " + line,
                                        e);
                }
                ...
                Pedido pedido = Pedido.builder()
                                ...
                                .build();
                // NOTA: No agregamos el pedido a la lista aquí, el DataLoader se encarga de
                // esto
                // pedidos.add(pedido);
                return pedido;
        }
        ...
        /**
         * Crea un pedido a partir de una línea de texto (método legacy para
         * compatibilidad).
         *
         * @param line Línea de texto con los datos del pedido
         * @return instancia de Pedido creada
        ...
        public static Pedido crearPedido(String line) throws InvalidDataFormatException {
                // Usar el método actualizado con mes por defecto (febrero para compatibilidad)
                return crearPedido(line, "ventas202502.txt");
        }
