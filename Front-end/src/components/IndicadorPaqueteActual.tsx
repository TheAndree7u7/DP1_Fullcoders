...
const IndicadorPaqueteActual: React.FC<IndicadorPaqueteActualProps> = ({
  ...
  const { paqueteActualConsumido, fechaHoraSimulacion, simulacionActiva } = useSimulacion();

  // Cargar información inicial solo una vez para obtener el total de paquetes
  useEffect(() => {
    const cargarInfoInicial = async () => {
      try {
        const info = await obtenerInfoSimulacion();
        setInfoSimulacion(info);
        setError('');
      } catch (err) {
        console.error('Error al obtener info inicial de simulación:', err);
  ...
  // Actualizar el paquete actual cuando cambie en el contexto
  useEffect(() => {
    if (infoSimulacion) {
      setInfoSimulacion(prev => prev ? {
        ...prev,
        paqueteActual: paqueteActualConsumido,
        enProceso: simulacionActiva,
        tiempoActual: fechaHoraSimulacion || prev.tiempoActual
      } : null);
    }
  }, [paqueteActualConsumido, simulacionActiva, fechaHoraSimulacion]);
